...
def getmappingfile():
    ...
    """
    return pd.read_pickle('mapdf.pickle')
...
def getelixhauser_comorbidities():
    ...
    Comments:
        N/A
    """
    return loadobj('elixhauser_comorbidities.pickle')
...
def icdtophenotype(datain,idcolumn=None,icdcolumn=None,featurematrix=False):
    ...
    for como in elixhauser_comorbidities:
        for icdcode in elixhauser_comorbidities[como]['icd9']+elixhauser_comorbidities[como]['icd10']:
            print('Currently processing: '+como+', code: '+icdcode+'                                                         ',end="\r",flush=True)
            dataout.loc[dataout[icdcolumn].str[0:len(icdcode)]==icdcode,'comorbidity']=como
    
    
    if not featurematrix:
        return dataout
    
    dataout=pd.concat([dataout[idcolumn],pd.get_dummies(dataout['comorbidity'])],axis=1).groupby('REG_NO').max()
